...
public class Frequencer implements FrequencerInterface {
  ...
  public void setSpace(byte[] space) {
    ...
    //
    quicksort(suffixArray, 0, suffixArray.length);
  }

  private void quicksort(int[] ary, int start, int end) {
    int l = start;
    int r = end - 1;
    int tmp = 0;
    int plot = suffixArray[(int) (end + start - 1) / 2];
    while (l <= r) {
      while (l < end && suffixCompare(plot, suffixArray[l]) > 0)
        l++;
      while (r > start && suffixCompare(plot, suffixArray[r]) < 0)
        r--;
      if (l > r)
        break;
      tmp = ary[l];
